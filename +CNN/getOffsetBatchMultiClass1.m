function [ y ] = getOffsetBatchMultiClass1( imdb,opts, batch  )
...
for k=1:length(batch)
    ind=batch(k);
    img= single(imdb.images.data(:,:,:,ind));
    mk=single(imdb.images.mask(:,:,[1,2,3],ind));
  %  o=single(imdb.offsets{ind});
    
   % img=single(imdb.images{ind});
%    mk=cat(3,o(:,:,[1,2]),single(imdb.labels{ind}));
    
    if(imdb.images.set(batch)==1)
         %crop
