function [net,stats] = cnn_train_dag_offset_and_class_and_size(net, imdb, getBatch, varargin)
...
% net.vars(predVar1).precious=true;
 
% predVar2 = net.getVarIndex('prediction2') ;
% net.vars(predVar2).precious=true;
 
% predVar3 = net.getVarIndex('prediction3') ;
% net.vars(predVar3).precious=true;
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    if numel(batch) == 0, continue ; end
     ...
     %s1=tic;
     opts.prefetch=false;
     inputs = state.getBatch(state.imdb,opts, batch) ;
     opts.prefetch=true;
     %timePrefetch = toc(s1);
    % disp(['Fetch Out Dur:', num2str(timePrefetch)]);
    if opts.prefetch
      ...
      %s2=tic;
      state.getBatch(state.imdb,opts, nextBatch) ;
     % timeFetch=toc(s2);
     % disp(['PreFetch Out Dur :', num2str(timeFetch)]);
    end
    ...
    if strcmp(mode, 'train')
      ...
      %evalTime=toc(s3);
      %disp(['Eval Out time :', num2str(evalTime)]);
     
      
      %{
      predVar = net.getVarIndex('prediction1') ;
      scores1_ = gather(net.vars(predVar).value);
      
      predVar = net.getVarIndex('prediction2') ;
      scores2_ = gather(net.vars(predVar).value);
      
      labelVar = net.getVarIndex('label') ;
      orig_scores_ = gather(net.vars(labelVar).value);
     
      f=figure ;
      subplot(2,3,1);
      imagesc(orig_scores_(:,:,1));
      title('Original x');
      
      subplot(2,3,2);
      imagesc(orig_scores_(:,:,2));
      title('Original y');
      
      subplot(2,3,3);
      imagesc(orig_scores_(:,:,3));
      title('Original probability');
  ...
  %disp(['Batch Time :', num2str(batchTime)]);
