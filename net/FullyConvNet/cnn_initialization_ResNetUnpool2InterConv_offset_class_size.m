function net = cnn_initialization_ResNetUnpool2InterConv_offset_class_size(varargin)
...
%% Prediction
%x_32x_ReLU
net.addLayer('ConvPred1', ...
    dagnn.Conv('size', [3,3,32,2], 'pad', 1, 'stride', 1), ...
    {'dropped'}, ...
    {'prediction1'}, ...
    {'ConvPred1_f','ConvPred1_b'});
params = net.params(net.layers(numel(net.layers)).paramIndexes);
[params.learningRate] = deal(opts.scale, opts.scale);
[params.weightDecay] = deal(opts.weightDecay(1), opts.weightDecay(1));
net.params(net.layers(numel(net.layers)).paramIndexes) = params;

net.addLayer('ConvPred2', ...
    dagnn.Conv('size', [3,3,32,21], 'pad', 1, 'stride', 1), ...
    {'dropped'}, ...
    {'prediction2'}, ...
    {'ConvPred2_f','ConvPred2_b'});
params = net.params(net.layers(numel(net.layers)).paramIndexes);
[params.learningRate] = deal(opts.scale, opts.scale);
[params.weightDecay] = deal(opts.weightDecay(1), opts.weightDecay(1));
net.params(net.layers(numel(net.layers)).paramIndexes) = params;
...
net.addLayer('ConvPred3', ...
    dagnn.Conv('size', [3,3,32,2], 'pad', 1, 'stride', 1), ...
    {'dropped'}, ...
    {'prediction3'}, ...
    {'ConvPred3_f','ConvPred3_b'});
params = net.params(net.layers(numel(net.layers)).paramIndexes);
[params.learningRate] = deal(opts.scale, opts.scale);
[params.weightDecay] = deal(opts.weightDecay(1), opts.weightDecay(1));
net.params(net.layers(numel(net.layers)).paramIndexes) = params;
...
net.addLayer('loss1',dagnn.L2LossAdaptive(), {'prediction1', 'label'}, 'objective1') ;
net.addLayer('loss2', dagnn.SegmentationLoss('loss', 'softmaxlog'),{'prediction2', 'label'}, 'objective2') ;
net.addLayer('accuracy',dagnn.SegmentationAccuracy(), {'prediction2', 'label'}, 'accuracy') ;
net.addLayer('loss3',dagnn.L2Loss(), {'prediction3', 'label'}, 'objective3') ;


%net.addLayer('error',dagnn.RMSError(), {'prediction1', 'label'}, 'error') ;
