classdef L2LossAdaptiveBoundary < dagnn.Loss
  ...
  methods
    function outputs = forward(obj, inputs, params)
      ...
      %background loss set to 0
      diff(~mask) = 0;
      %inside and outside loss set to 0
      diff(~boundary_masks)=0;
      
      %background pixels that point to foreground
      diff(mask1) = predictions(mask1);
      
      full_mask=boundary_masks|mask1;
