function [] = CNN_test_offset_class_voc()
 
        %to test actual PASCAL VOC detection validation set
        close all;
        clear;
        ...
        opts.epoch=486; %124;
        opts.resultDir=[opts.expDir filesep 'ResultsNow' filesep 'VOCval' filesep sprintf('%d',opts.epoch)];
...
VOCopts.annopath=[VOCPATH  filesep 'Annotations'];
VOCopts.imgpath=[VOCPATH filesep 'JPEGImages' filesep];
VOCopts.imgsetpath= fullfile(VOCPATH, 'ImageSets', 'Main' , 'val.txt');      
       ...
       [ids,h]=textread(VOCopts.imgsetpath,'%s %d');
