function [] = CNN_test_offset_class_size_4d_hough_voc()
        ...
        opts.expDir=[opts.baseDir filesep 'rgb_object_detection-offset-class-size-L-0.0001-normalised-L2Adaptive'];
       
        opts.epoch=1520; %124;
        opts.resultDir=[opts.expDir filesep 'Results' filesep 'VOCval07' filesep sprintf('%d',opts.epoch)];
...
switch(method)
    ...
    case 2  
           opts.threshold=5;
           opts.bin_size=[5,5];
           opts.size_bin_size=[16,16];
         
           opts.non_m_win_size=9;
           opts.size_non_m_win_size=3;
        
           post_process_pref=sprintf('4d_hough_th_%d_bin_%d_sbin_%d_win_%d_swin_%d_with_nmx1_conf_1_test_time',opts.threshold,opts.bin_size(1), opts.size_bin_size(1),opts.non_m_win_size, opts.size_non_m_win_size);
...
opts.imgsetpath= fullfile(VOCPATH, 'ImageSets', 'Main' , 'test.txt');
 ...
 for i=1:length(ids)
             ...
             switch method
               ...
               [centers_y,centers_x,widths,heights,contri]= find_centers_4dhough_voting(offset,bbox_size, opts.bin_size, opts.size_bin_size,opts.threshold,opts.non_m_win_size,opts.size_non_m_win_size);
                 case 3 
