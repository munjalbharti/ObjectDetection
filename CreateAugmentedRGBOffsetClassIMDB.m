function CreateAugmentedRGBOffsetClassIMDB(  )
...
opts.target_file = [base_dir filesep 'data' filesep 'rgb_object_detection-15-person-offset-class-1-bkg-NaN-boundary-imdb.mat'];
...
for set_no=1:1
    if(set_no==1)
        ...
        offset_dir=fullfile(imdb.paths.mask ,'offset_class_bkg_nan_boundaries1','train');
        files_info = dir([masks_dir filesep '*.png']);
    else
        masks_dir=fullfile(imdb.paths.mask,'val'); 
        offset_dir=fullfile(imdb.paths.mask ,'offset_class_bkg_nan_boundaries1','val');
        files_info = dir([masks_dir filesep '*.png']);
...
for file_no=1:1
    ...
    [seg_obj,seg_map]=imread(fullfile(seg_obj_dir,filename));
     ...
     [offset,obj_rects,boundary]= generate_offset_and_class_and_boundary_mask(In_cls,seg_obj);
      %save offset only for non-augmented data 
     save_offset_and_boundary(In_cls,offset,obj_rects,boundary,offset_dir,filename(1:end-4) )
     ...
     imdb.images.mask(:,:,:,count) = cat(3,offset,cat(3,boundary,boundary));
     
    
     imdb.images.set(count) =set_no;
     %imdb.images.boundary_mask(:,:,:,:,count)=;
     count=count+1;
...
if(set_no ==1)
     for k=1:no_of_aug_per_img
        ...
        [offset_aug,obj_rects_aug,boundary_aug]= generate_offset_and_class_and_boundary_mask(mask_aug,obj_aug);
       ...
       % imdb.images.mask(:,:,:,count) =offset_aug;
        imdb.images.mask(:,:,:,count) = cat(3,offset_aug,cat(3,boundary_aug,boundary_aug)); 
        imdb.images.set(count) =set_no;
        %imdb.images.boundary_mask(:,:,:,:,count)=cat(3,offset,boundary);
