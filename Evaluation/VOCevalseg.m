...
% image test set
[gtids,t]=textread(VOCopts.seg.imgsetpath,'%s %d');
...
for i=1:length(gtids)
    ...
    gtfile = fullfile(VOCopts.seg.clsimgpath,sprintf('%s.png',imname));
    %gtfile=fullfile(comp5_val_cls,imname);
    [gtim,map1] = imread(gtfile);    
    gtim = double(gtim);
    
    % results file
    resfile = fullfile(VOCopts.seg.clsresdir,sprintf('%s.png',imname));
   % resfile= fullfile(VOCopts.seg.clsrespath);
    [resim,map2] = imread(resfile);
    resim = double(resim);
