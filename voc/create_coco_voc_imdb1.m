function create_coco_voc_imdb1(  )
...
opts.target_file = [base_dir filesep 'rgb_object_detection-coco-voc-imdb-ssd.mat'];
count=0;
...
for set_no=1:2
    for dataset=1:2
        ...
        if(set_no==1)
            ...
            offset_fold='train_offsets';
            labl_size_fold='train_offsets';
        else
            img_fold='val_images';
            labl_fold='val_labels';
            offset_fold='val_offsets';
            labl_size_fold='val_offsets';
        end
    ...
    for k=1:length(files_info)
         ...
         imdb.images.label_filenames{count}= fullfile(base_dir,dataset_folder,labl_fold,sprintf('%s.png',filename(1:end-4)));
