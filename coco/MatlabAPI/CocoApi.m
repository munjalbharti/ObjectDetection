classdef CocoApi
  ...
  methods
    ...
    function ids = getImgIds( coco, varargin )
      ...
      %Added by Bharti
      ids=[];
      for i=1:length(t), ids=union(ids,t{i});end
    end
    ...
    function all_masks = getSegmentationMasks(coco, anns,img)
        ...
        % img = coco.loadImgs(imgId);
         ...
         all_masks= zeros(img.height,img.width,n);
         for i=1:n
          if(anns(i).iscrowd)
              C=[.01 .65 .40]; 
          else
          ...
          if(isstruct(S{i}))
              ...
              %cu_mask_disp=zeros(img.height,img.width,'uint8');
              cu_mask=zeros(img.height,img.width,'logical');
              
              for j=1:length(S{i})
                  P=S{i}{j}+.5;
                  k=k+1;               
                  BW = poly2mask(P(1:2:end),P(2:2:end),img.height, img.width);
                  cu_mask=cu_mask | BW;                    
              end
                
               all_masks(:,:,i) = cu_mask * cat_ids{i};
          end
         end
    ...
    function all_masks = getSegmentationMasks_back(coco, anns,imgId)
         n=length(anns); if(n==0), return; end
         S={anns.segmentation};
         ...
         hs=zeros(10000,1);
         k=0; %hold on;
         pFill={'FaceAlpha',0.4,'LineWidth',3};
         img = coco.loadImgs(imgId);
          
         all_masks= zeros(img.height,img.width,n,'uint8');
         for i=1:n
          if(anns(i).iscrowd)
              C=[.01 .65 .40]; 
          else
          ...
          if(isstruct(S{i}))
              ...
              %cu_mask_disp=zeros(img.height,img.width,'uint8');
              cu_mask=zeros(img.height,img.width,'uint8');

              f3=figure;
              imshow(cu_mask); hold on;              
              
              for j=1:length(S{i})
                  P=S{i}{j}; %+.5;
                  k=k+1;
                  hs(k)=fill(P(1:2:end),P(2:2:end),[1,0,0],pFill{:});
                  hold on ;                      
              end
              ...
              im=F.cdata;
              [y,x]=find(im(:,:,1) ~= 0);
              ind1=find(y <= img.height & x <= img.width );
             
              y1=y(ind1);
              x1=x(ind1);

              ind=sub2ind([img.height,img.width],y1,x1);
             % for p=1:length(ind_y)
              %      cu_mask(ind_y(p),ind_x(p))=1;
              %end 
              cu_mask(ind)=1;
